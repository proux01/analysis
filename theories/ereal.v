...
Definition dual_adde := nosimpl dual_adde_def.
...
Notation "*%E"   := mule.
Notation "x * y" := (mule x%dE y%dE) : ereal_dual_scope.
...
Notation "\sum_ ( i <- r | P ) F" :=
  (\big[+%dE/0%:E]_(i <- r | P%B) F%E) : ereal_dual_scope.
Notation "\sum_ ( i <- r | P ) F" :=
  (\big[+%E/0%:E]_(i <- r | P%B) F%E) : ereal_scope.
Notation "\sum_ ( i <- r ) F" :=
  (\big[+%dE/0%:E]_(i <- r) F%E) : ereal_dual_scope.
Notation "\sum_ ( i <- r ) F" :=
  (\big[+%E/0%:E]_(i <- r) F%E) : ereal_scope.
Notation "\sum_ ( m <= i < n | P ) F" :=
  (\big[+%dE/0%:E]_(m <= i < n | P%B) F%E) : ereal_dual_scope.
Notation "\sum_ ( m <= i < n | P ) F" :=
  (\big[+%E/0%:E]_(m <= i < n | P%B) F%E) : ereal_scope.
Notation "\sum_ ( m <= i < n ) F" :=
  (\big[+%dE/0%:E]_(m <= i < n) F%E) : ereal_dual_scope.
Notation "\sum_ ( m <= i < n ) F" :=
  (\big[+%E/0%:E]_(m <= i < n) F%E) : ereal_scope.
Notation "\sum_ ( i | P ) F" :=
  (\big[+%dE/0%:E]_(i | P%B) F%E) : ereal_dual_scope.
Notation "\sum_ ( i | P ) F" :=
  (\big[+%E/0%:E]_(i | P%B) F%E) : ereal_scope.
Notation "\sum_ i F" :=
  (\big[+%dE/0%:E]_i F%E) : ereal_dual_scope.
Notation "\sum_ i F" :=
  (\big[+%E/0%:E]_i F%E) : ereal_scope.
Notation "\sum_ ( i : t | P ) F" :=
  (\big[+%dE/0%:E]_(i : t | P%B) F%E) (only parsing) : ereal_dual_scope.
Notation "\sum_ ( i : t | P ) F" :=
  (\big[+%E/0%:E]_(i : t | P%B) F%E) (only parsing) : ereal_scope.
Notation "\sum_ ( i : t ) F" :=
  (\big[+%dE/0%:E]_(i : t) F%E) (only parsing) : ereal_dual_scope.
Notation "\sum_ ( i : t ) F" :=
  (\big[+%E/0%:E]_(i : t) F%E) (only parsing) : ereal_scope.
Notation "\sum_ ( i < n | P ) F" :=
  (\big[+%dE/0%:E]_(i < n | P%B) F%E) : ereal_dual_scope.
Notation "\sum_ ( i < n | P ) F" :=
  (\big[+%E/0%:E]_(i < n | P%B) F%E) : ereal_scope.
Notation "\sum_ ( i < n ) F" :=
  (\big[+%dE/0%:E]_(i < n) F%E) : ereal_dual_scope.
Notation "\sum_ ( i < n ) F" :=
  (\big[+%E/0%:E]_(i < n) F%E) : ereal_scope.
Notation "\sum_ ( i 'in' A | P ) F" :=
  (\big[+%dE/0%:E]_(i in A | P%B) F%E) : ereal_dual_scope.
Notation "\sum_ ( i 'in' A | P ) F" :=
  (\big[+%E/0%:E]_(i in A | P%B) F%E) : ereal_scope.
Notation "\sum_ ( i 'in' A ) F" :=
  (\big[+%dE/0%:E]_(i in A) F%E) : ereal_dual_scope.
Notation "\sum_ ( i 'in' A ) F" :=
...
Lemma defined_dual_addeE x y : adde_defined x y -> (x + y)%dE = (x + y)%E.
...
Lemma lee_subl_addr x r z : (x - r%:E <= z) = (x <= z + r%:E).
Proof.
...
Lemma gee_pmull r x : 0 < x -> (r <= 1)%R -> r%:E * x <= x.
Proof.
move: x => [x| |] //=.
...
Lemma lte_spaddr r x y : 0 < y -> r%:E <= x -> r%:E < x + y.
Proof.
...
- move: (x0); rewrite lee_fin le_eqVlt => /orP[/eqP <-|{}x0].
...
- rewrite le_eqVlt => /orP[/eqP <-|]; last by rewrite lte_pdivr_mull// => /ltW.
...
Proof.
move=> r0; apply/idP/idP.
- rewrite le_eqVlt => /orP[/eqP ->|]; last by rewrite lte_pdivl_mull// => /ltW.
  by rewrite muleA -mulEFin divrr ?mul1e// unitfE gt_eqF.
- rewrite le_eqVlt => /orP[/eqP <-|]; last by rewrite -lte_pdivl_mull// => /ltW.
  by rewrite muleA -mulEFin mulVr ?mul1e// unitfE gt_eqF.
Qed.
