...
by rewrite gt_eqF// (lt_le_trans ltNy0 (abse_ge0 t)).
Qed.
...
Definition keep_nonpos_itv_bound_subdef (b : itv_bound int) : itv_bound int :=
  match b with
  | BSide b (Posz 0) => BRight 0%Z
...
apply/and3P; split.
- by rewrite -real_leey ub_ereal_sup// => _ [[[x||] /=/and3P[? ? ?]] _ <-].
- by case: ereal_sup.
...
Definition keep_nonneg_itv_bound_subdef (b : itv_bound int) : itv_bound int :=
  match b with
  | BSide b (Posz _) => BLeft 0%Z
  | BSide _ (Negz _) => -oo%O
  | BInfty _ => -oo%O
  end.
...
rewrite {}/r; case: i S => [//| [l u]] S /=.
apply/and3P; split.
- by rewrite -real_leNye lb_ereal_inf// => _ [[[x||] /=/and3P[? ? ?]] _ <-].
