...
From mathcomp Require Import mathcomp_extra itv.
...
Canonical pinfty_inum := Itv.mk (pinfty_spec).

Lemma ninfty_spec : ext_num_spec (Itv.Real `]-oo, (-1)%Z[) (-oo : \bar R).
Proof. by apply/and3P; rewrite /= cmp0Ny !bnd_simp real_ltNyr. Qed.

Canonical ninfty_snum := Itv.mk (ninfty_spec).

Lemma EFin_spec i (x : num_def R i) : ext_num_spec i x%:num%:E.
Proof.
...
Definition fine_itv i :=
  let: Interval l u := i in
  Interval (keep_nonneg_itv_bound l) (keep_nonpos_itv_bound u).

Lemma fine_spec i (x : ext_num_def i) (r := itv_real1_subdef fine_itv i) :
...
Canonical fine_inum i (x : ext_num_def i) := Itv.mk (fine_spec x).
...
Lemma oppe_itv_boundr (x : \bar R) b :
  (BRight (- x) <= ext_num_itv_bound (opp_itv_bound b))%O
  = (ext_num_itv_bound b <= BLeft x)%O.
Proof.
by case: b => [[] b | []//]; rewrite /= !bnd_simp mulrNz EFinN ?leeN2 // lteN2.
Qed.

Lemma oppe_itv_boundl (x : \bar R) b :
  (ext_num_itv_bound (opp_itv_bound b) <= BLeft (- x))%O
  = (BRight x <= ext_num_itv_bound b)%O.
Proof.
by case: b => [[] b | []//]; rewrite /= !bnd_simp mulrNz EFinN ?leeN2 // lteN2.
Qed.

Lemma oppe_spec i (x : ext_num_def i)
...
by rewrite -[x]/(Itv.mk Px)%:inum opp_spec.
Qed.

Canonical oppe_inum i (x : ext_num_def i) := Itv.mk (oppe_spec x).

Lemma adde_spec xi yi (x : ext_num_def xi) (y : ext_num_def yi)
...
by rewrite -[x]/(Itv.mk Px)%:inum -[y]/(Itv.mk Py)%:inum add_spec.
Qed.

Canonical adde_inum xi yi (x : ext_num_def xi) (y : ext_num_def yi) :=
  Itv.mk (adde_spec x y).

Import DualAddTheory.

Lemma dEFin_spec i (x : num_def R i) : ext_num_spec i (dEFin x%:num).
Proof.
case: i x => [//| [l u] [x /=/and3P[xr /= lx xu]]].
by apply/and3P; split=> [//||]; [case: l lx | case: u xu].
Qed.

Canonical dEFin_inum i (x : num_def R i) := Itv.mk (dEFin_spec x).

Lemma dadde_spec xi yi (x : ext_num_def xi) (y : ext_num_def yi)
    (r := itv_real2_subdef add_itv xi yi) :
...
Lemma ext_mul_itv_boundl_spec b1 b2 (x1 x2 : \bar R) :
  (BLeft 0%:Z <= b1 -> BLeft 0%:Z <= b2 ->
   ext_num_itv_bound b1 <= BLeft x1 ->
   ext_num_itv_bound b2 <= BLeft x2 ->
   ext_num_itv_bound (mul_itv_boundl b1 b2) <= BLeft (x1 * x2))%O.
Proof.
...
Lemma ext_mul_itv_boundr_spec b1 b2 (x1 x2 : \bar R) :
  (0 <= x1 -> 0 <= x2 ->
   BRight x1 <= ext_num_itv_bound b1 ->
   BRight x2 <= ext_num_itv_bound b2 ->
   BRight (x1 * x2) <= ext_num_itv_bound (mul_itv_boundr b1 b2))%O.
Proof.
...
- rewrite !(map_itv_bound_comp, BRight_EFin_le_map_itv_bound)/=.
  exact: mul_itv_boundr_spec.
- rewrite real_mulry// => b1x1 b2x2.
  have -> : b2 = +oo%O by case: b2 b2x2 => -[].
  rewrite mul_itv_boundrC/= map_itv_bound_comp.
  case: (comparable_ltgtP x1r) x10 => [x10 |//| [x10]] _.
...
Lemma ext_mul_itv_boundr'_spec b1 b2 (x1 x2 : \bar R) :
...
Lemma mule_spec xi yi (x : ext_num_def xi) (y : ext_num_def yi)
...
- case: (ext_interval_signP yly yyu yr) => ylb yub ys.
  + by rewrite ys mule0.
  + apply/and3P; split=> //=.
    * exact: ext_mul_itv_boundl_spec.
    * exact: ext_mul_itv_boundr_spec.
  + apply/and3P; split=> //=; rewrite -[x * y]oppeK -real_muleN//.
    * rewrite oppe_itv_boundl.
      by rewrite ext_mul_itv_boundr_spec ?oppe_ge0 ?oppe_itv_boundr.
    * rewrite oppe_itv_boundr.
      rewrite ext_mul_itv_boundl_spec ?oppe_itv_boundl//.
      by rewrite opp_itv_ge0.
  + apply/and3P; split=> //=.
    * rewrite  -[x * y]oppeK -real_muleN// oppe_itv_boundl.
      rewrite ext_mul_itv_boundr'_spec -?real_fine ?oppe_cmp0
              ?oppe_itv_boundr//.
      by rewrite opp_itv_gt0 ltW.
    * by rewrite ext_mul_itv_boundr'_spec// ltW.
- case: (ext_interval_signP yly yyu yr) => ylb yub ys.
  + by rewrite ys mule0.
  ...
  + apply/and3P; split=> //=; rewrite -real_muleNN//.
    * by rewrite ext_mul_itv_boundl_spec ?opp_itv_ge0 ?oppe_itv_boundl.
    * by rewrite ext_mul_itv_boundr_spec ?oppe_ge0 ?oppe_itv_boundr.
  + apply/and3P; split=> //=; rewrite -[x * y]oppeK.
    * rewrite -real_mulNe// oppe_itv_boundl.
      rewrite ext_mul_itv_boundr'_spec ?oppe_ge0 ?oppe_itv_boundr//.
      exact: ltW.
    * rewrite oppeK -real_muleNN//.
      by rewrite ext_mul_itv_boundr'_spec ?oppe_itv_boundr
                 ?oppe_ge0 ?oppe_cmp0 ?opp_itv_gt0// ltW.
case: (ext_interval_signP yly yyu yr) => ylb yub ys.
- by rewrite ys mule0.
- apply/and3P; split=> //=; rewrite muleC mul_itv_boundrC.
  + rewrite -[y * x]oppeK -real_muleN// oppe_itv_boundl.
    rewrite ext_mul_itv_boundr'_spec ?oppe_ge0 ?oppe_cmp0 ?oppe_itv_boundr//.
    by rewrite opp_itv_gt0 ltW.
  + by rewrite ext_mul_itv_boundr'_spec// ltW.
- apply/and3P; split=> //=; rewrite muleC mul_itv_boundrC.
  + rewrite -[y * x]oppeK -real_mulNe// oppe_itv_boundl.
    rewrite ext_mul_itv_boundr'_spec ?oppe_ge0 ?oppe_itv_boundr//.
    exact: ltW.
  + rewrite -real_muleNN// ext_mul_itv_boundr'_spec ?oppe_ge0
            ?oppe_cmp0 ?oppe_itv_boundr//.
    by rewrite opp_itv_gt0 ltW.
...
- apply/orP; right; rewrite -[x * y]oppeK -real_muleN// oppe_itv_boundl.
  rewrite ext_mul_itv_boundr'_spec ?oppe_cmp0 ?oppe_itv_boundr//.
  by rewrite opp_itv_gt0 ltW.
- by apply/orP; right; rewrite ext_mul_itv_boundr'_spec// ltW.
- apply/orP; left; rewrite -[x * y]oppeK -real_mulNe// oppe_itv_boundl.
  rewrite ext_mul_itv_boundr'_spec ?oppe_ge0 ?oppe_itv_boundr//.
  exact: ltW.
...
Lemma abse_spec i (x : ext_num_def i) (r := abse_itv i) :
  ext_num_spec r (`|x%:inum| : \bar R).
Proof.
...
Canonical abse_inum i (x : ext_num_def i) := Itv.mk (abse_spec x).
...
Lemma ext_min_spec (xi yi : Itv.t) (x : ext_num_def xi) (y : ext_num_def yi)
...
Proof.
case: (leP b1 b2) => [b1_le_b2 | /ltW b2_le_b1].
- have sb1_le_sb2 := eqbRL (@ext_le_map_itv_bound R _ _) b1_le_b2.
  by rewrite maxElt; case: ifP => [//|_ ? _]; apply: le_trans sb1_le_sb2.
- have sb2_le_sb1 := eqbRL (@ext_le_map_itv_bound R _ _) b2_le_b1.
  by rewrite maxElt; case: ifP => [_ _ ?|//]; apply: le_trans sb2_le_sb1.
Qed.

Lemma ext_max_spec (xi yi : Itv.t) (x : ext_num_def xi) (y : ext_num_def yi)
...
Canonical ext_min_max_typ (R : numDomainType) :=
  MinMaxTyp ext_min_spec ext_max_spec.
